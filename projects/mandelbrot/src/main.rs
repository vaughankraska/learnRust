...
fn test_parse_complex() {
    assert_eq!(parse_complex("1.25, -0.073"), Some(Complex { re: 1.25, im: -0.073 }));
    assert_eq!(parse_complex(", -0.41"), None);
}
...
fn write_image(
    ...
    // above line is equivalent to: (but the ? is shorthand for it)
    // let output = match File::create(filename) {
    //      Ok(f) => f,
    //      Err(e) => { return Err(e); }
    // };
    let encoder = PNGEncoder::new(output);
    encoder.encode(pixels, bounds.0 as u32, bounds.1 as u32, ColorType::Gray(8))?; // note the ? again
